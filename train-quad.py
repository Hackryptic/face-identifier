...
def getQuadLoss(anchor, positive, negative, negative2, num_image_per_class, num_people_per_batch, alpha, alpha2, device):
    loss  = losses.quadruplet_loss(anchor, positive, negative, negative2, num_image_per_class, num_people_per_batch, alpha, alpha2, device)
    return loss
        ...
        for phase in ['train']:
            ...
            for idx, (labels, paths) in enumerate(blockloader):
                ...
                with torch.set_grad_enabled(False):
                    ...
                    if epoch < adapt_start_epoch:
                        quad_list = quadruplet.select_quads(embeddings, paths, num_for_class, class_per_batch, alpha, alpha2, device)
                    else:
                        print("adapt")
                        alpha, alpha2, quad_list = quadruplet.select_Adaquads(embeddings, paths, num_for_class, class_per_batch, alpha, alpha2, device)


                    quad_num = len(quad_list)
                ...
                total_quad_num += quad_num

                batchset = customdataset.ImageFolderQuads(quadruplet=quad_list, transform=test_transform)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    for idx, (anchor_image, positive_image, negative_image, negatives2_image) in enumerate(batchloader):
                        ...
                        negatives2_image = negatives2_image.to(device)

                                                
                        anchors = model(anchor_image)
                        positives = model(positive_image)
                        negatives = model(negative_image)
                        negatives2 = model(negatives2_image)
                        

                        loss  = criterion(anchors, positives, negatives, negatives2, num_for_class, class_per_batch, alpha, alpha2, device)
